...
class TelmsgController extends AdminBaseController {
	private $_where = array();//查询条件
	private $_data = array();//数据数组
	private $_db = 'telmsg';//数据库
	private $_name = '';
	/***
	...
	public function admin_add(){
		...
		if(IS_POST){
			...
			$value['typeid'] = $this->I_chk('post.typeid','模板id',0,0);
			$value['data'] = $this->I_chk('post.data','发送数据',0,0);
			$value['usermobile'] = $this->I_chk('post.usermobile','用户手机',0,0);
			$value['state'] = $this->I_chk('post.state','数据发送状态',0,0);
			$value['pin'] = $this->I_chk('post.pin','验证码',0,0);
			$value['date'] = $this->I_chk('post.date','发送日期',0,0);
			$value['datetime'] = $this->I_chk('post.datetime','发送时间',0,0);
