...
class OrderstyleController extends AdminBaseController {
	private $_where = array();//查询条件
	private $_data = array();//数据数组
	private $_db = 'orderstyle';//数据库
	private $_name = '';
	/***
	...
	public function admin_add(){
		...
		if(IS_POST){
			...
			$value['groupid'] = $this->I_chk('post.groupid','用户组',0,0);
			$value['adminid'] = $this->I_chk('post.adminid','管理组',0,0);
			$value['name'] = $this->I_chk('post.name','事由详情',0,0);
