...
class JifenlogController extends AdminBaseController {
	private $_where = array();//查询条件
	private $_data = array();//数据数组
	private $_db = 'jifenlog';//数据库
	private $_name = '';
	/***
	...
	public function admin_add(){
		...
		if(IS_POST){
			...
			$value['adminid'] = $this->I_chk('post.adminid','操作员id',0,0);
			$value['typeid'] = $this->I_chk('post.typeid','积分项目',0,0);
			$value['name'] = $this->I_chk('post.name','积分详情',0,0);
			$value['date'] = $this->I_chk('post.date','积分日期',0,0);
			$value['datetime'] = $this->I_chk('post.datetime','积分时间',0,0);
			$value['jifen'] = $this->I_chk('post.jifen','积分',0,0);
			$value['beginjifen'] = $this->I_chk('post.beginjifen','开始值',0,0);
			$value['endjifen'] = $this->I_chk('post.endjifen','结束值',0,0);
