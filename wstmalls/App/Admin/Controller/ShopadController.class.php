...
class ShopadController extends AdminBaseController {
	private $_where = array();//查询条件
	private $_data = array();//数据数组
	private $_db = 'shopad';//数据库
	private $_name = '';
	/***
	...
	public function admin_ShowList(){
		...
		if(isset($search['shopid'])){$this->_where['shopid'] = $search['shopid'];}
		if(isset($search['type'])){$this->_where['type'] = $search['type'];}
		if(isset($search['name'])){$this->_where['name'] = $search['name'];}
		if(isset($search['img'])){$this->_where['img'] = $search['img'];}
		if(isset($search['link'])){$this->_where['link'] = $search['link'];}
	...
	public function admin_add(){
		...
		if(IS_POST){
			...
			$value['shopid'] = $this->I_chk('post.shopid','商城id',0,0);
			$value['type'] = $this->I_chk('post.type','广告组',0,0);
			$value['name'] = $this->I_chk('post.name','广告名称',0,0);
			$value['img'] = $this->I_chk('post.img','广告图片',0,0);
			$value['link'] = $this->I_chk('post.link','广告链接',0,0);
			$value['sort'] = $this->I_chk('post.sort','广告排序',0,0);
			$value['state'] = $this->I_chk('post.state','广告状态',0,0);
			$value['about'] = $this->I_chk('post.about','拒绝原因',0,128);
