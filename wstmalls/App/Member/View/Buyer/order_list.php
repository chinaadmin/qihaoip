...
您当前的位置：<a href="<?php echo U('index'); ?>">买家中心</a> > <a href="<?php echo U('Buyer/order_list');?>">我的订单</a>
</div>
<div class="hgrzy">
<div class="col-xs-12 chgsc listweizhi">
<ul>
		<li><a href="<?php echo U('Buyer/order_list');?>" <?php if($data['type'] == 0){?>class="vselst1"<?php }?>>全部订单</a></li>
		<li><a href="<?php echo U('Buyer/order_list');?>?type=1" <?php if($data['type'] == 1){?>class="vselst1"<?php }?>>待支付</a></li>
		<li><a href="<?php echo U('Buyer/order_list');?>?type=2" <?php if($data['type'] == 2){?>class="vselst1"<?php }?>>已支付</a></li>
		<li><a href="<?php echo U('Buyer/order_list');?>?type=3" <?php if($data['type'] == 3){?>class="vselst1"<?php }?>>手续办理</a></li>
		<li><a href="<?php echo U('Buyer/order_list');?>?type=4" <?php if($data['type'] == 4){?>class="vselst1"<?php }?>>待确认</a></li>
		<li><a href="<?php echo U('Buyer/order_list');?>?type=5" <?php if($data['type'] == 5){?>class="vselst1"<?php }?>>已完成</a></li>
		<li><a href="<?php echo U('Buyer/order_list');?>?type=11" <?php if($data['type'] == 11){?>class="vselst1"<?php }?>>已超时</a></li>
		<li><a href="<?php echo U('Buyer/order_list');?>?type=12" <?php if($data['type'] == 12){?>class="vselst1"<?php }?>>已撤销</a></li>
</ul>
</div>
	...
	<div class="col-xs-12" style="border-style: none solid solid solid;border-color: #D1D1D1;border-width: 1px;line-height: 84px;">
	  ...
	  <div class="col-xs-8">
	    <div class="col-xs-5" style="display: block; font-weight: normal; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; line-height:20px;">
	  	...
	  	<a href="__APP__/trademark/<?php echo $value['sid']?>HTMLSUFFIX">
	  		<img style="border:1px solid #EAEAEA; margin: 10px 20px 10px 10px;width: 70px;height: 60px; float:left;" src="<?php $img = explode(',', $value['img']);echo $img[0]; ?>">
	  	...
	  	<a href="__APP__/patent/<?php echo $value['sid']?>HTMLSUFFIX">
	  		<img style="border:1px solid #EAEAEA; margin: 10px 20px 10px 10px;width: 70px;height: 60px; float:left;" src="<?php $img = explode(',', $value['img']);echo $img[0]; ?>">
	  		<p style="height:5px; overflow:hidden;">&nbsp;</p>申请号：<?php echo $value['tmno']; ?><br /><span style="color: #FF6600;">【<?php echo C('ITEM_PATM')[$value['tmpa']] ?>】</span><?php echo $value['title']; ?>
	  	</a>
	  	<?php }?>
	  	</div>
	  	<div class="col-xs-3 posi_tion"><?php echo $value['name']?$value['name']:$value['username']; ?>
	  ...
	  <div class="col-xs-12" style="border-right: solid 1px;border-bottom: dotted 1px;border-color: #D1D1D1;">
	  	<div class="col-xs-8" style="line-height: 20px ;float: left;text-overflow:ellipsis;overflow:hidden;white-space:nowrap;">
	  	<img src="<?php echo $row['picture']; ?>" style="border:solid 1px;border-color: #EAEAEA;margin: 10px 20px 10px 10px;width: 70px;height: 60px;  float:left;">
	  	<p style="height:5px; overflow:hidden;">&nbsp;</p>
	  	专利号：<?php echo $row['applynum']; ?><br /><span style="color: #FF6600;">【专利】</span><?php echo $row['cname']; ?></div>
		<div class="col-xs-2" style="line-height: 20px;margin-top: 20px;">第<?php echo $row['annual']; ?>年<br>年费续缴</div>
		<div class="col-xs-2" style="color: #FF6600;"><?php echo $row['pay_total']; ?></div>
	  </div>
	  	<?php } ?>
	  </div>
	  <?php } ?>
	  <div class="col-xs-4" >
		<div class="col-xs-5" style="line-height: 22px;padding-top:20px;"><span style="color: #FF6600;"><?php echo C('ORDER_STATE')[$value['state']]; ?></span><br/><?php echo '<a href="'.U('order_show',['uid'=>$value['uid']]).'">订单详情</a>'; ?></div>
		...
		if($value['state']=='1'){
			...
			echo '<br /><a href="'.U('/Index/Cart/pay',['uid'=>$value['uid']]).'" target="_blank" style="margin-top:-15px;float:left;"><span class="btn btn-warning">&nbsp;&nbsp;付款&nbsp;&nbsp;</span></a>'; 
			echo '<br /><a href="'.U('order_unset',['uid'=>$value['uid']]).'" target="_blank" style="margin-top:-15px;">取消订单</a>';
			echo "
		...
		";
			...
			echo '<a href="'.U('order_success',['uid'=>$value['uid']]).'" target="_blank"><span class="btn btn-success">&nbsp;&nbsp;确认&nbsp;&nbsp;</span></a>';
