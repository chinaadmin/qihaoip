...
class PersonController extends AdminBaseController {
	...
	public function admin_add(){
		...
		if(IS_POST){
			...
			$value['name'] = $this->I_chk('post.name','用户姓名',0,0);
			$value['sex'] = $this->I_chk('post.sex','性别',0,0);
			$value['cardid'] = $this->I_chk('post.cardid','身份号',0,0);
			$value['img1'] = $this->I_chk('post.img1','身份证正面',0,0);
			$value['img2'] = $this->I_chk('post.img2','身份证反面',0,0);
			$value['img3'] = $this->I_chk('post.img3','手持身份照',0,0);
