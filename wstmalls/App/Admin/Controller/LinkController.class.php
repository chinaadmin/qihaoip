...
class LinkController extends AdminBaseController {
	...
	public function admin_ShowList(){
		...
		if(isset($search['nofollow'])){$this->_where['nofollow'] = $search['nofollow'];}
	...
	public function admin_add(){
		...
		if(IS_POST){
			...
			$value['groupid'] = $this->I_chk('post.groupid','链接分组',0,0);
			$value['name'] = $this->I_chk('post.name','网站名称',0,0);
			$value['link'] = $this->I_chk('post.link','网址',0,0);
			$value['img'] = $this->I_chk('post.img','图片',0,0);
			$value['title'] = $this->I_chk('post.title','title标签',0,0);
			$value['nofollow'] = $this->I_chk('post.nofollow','nofollow标签',0,0);
			$value['alt'] = $this->I_chk('post.alt','alt标签',0,0);
			$value['about'] = $this->I_chk('post.about','关于',0,0);
