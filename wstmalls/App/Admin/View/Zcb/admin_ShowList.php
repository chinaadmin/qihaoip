...
<!-- content -->
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
          <ol class="breadcrumb">
  			...
  			<li><span>知产包管理</span></li>
  			<li class="active">知产包列表&nbsp;&nbsp;|&nbsp;&nbsp;<a href="<?php echo U('admin_add'); ?>" target="_blank">新增知产包</a></li>
		  </ol>
          <div class="panel panel-default">
		  <div class="panel-heading">
<form class="form-inline" action="<?php echo U('admin_ShowList'); ?>" id="form-search" method="post">
  <div class="form-group">
    <label for="Admin-Zcb-id">ID</label>
    <input type="text" class="form-control input-sm" name="id" id="Admin-Zcb-id" <?php if(isset($search['id'])){echo 'value="'.$search['id'].'"'; } else {echo 'placeholder="数据id"'; } ?>>
  </div>
  <div class="form-group">
    <label for="Admin-Zcb-type">分类</label>
    <select class="form-control" id="Admin-Zcb-type" name="type">
    <option value ="0">未选择</option>
<?php get_select('items', $search['type']) ?>
	</select>
  </div>  <div class="form-group">
    <label for="Admin-Zcb-title">标题</label>
    <input type="text" class="form-control input-sm" name="title" id="Admin-Zcb-title" <?php if(isset($search['title'])){echo 'value="'.$search['title'].'"'; } else {echo 'placeholder="数据详情"'; } ?>>
  </div>
