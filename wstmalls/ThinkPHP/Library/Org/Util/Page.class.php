...
class Page{
    ...
    public function __construct($count,$pagesize){
        ...
        //$this->page    = empty($_GET[$this->p]) ? 1 : intval($_GET[$this->p]);
    ...
    public function getPage(){
        ...
        $pagestr = '';
         ...
         if($this->page == 1){
         	$pagestr.='<div class="daohan daohan_bottom">';
         	$pagestr.='<div class="qiehuan">';
        ...
        if($this->page > 1){
        	...
        	$pagestr.='<div class="qiehuan">';
        	if($this->page > 5){
        		$pagestr.= '<div class="qieleft"><a href='.$url.'p=1'.'>首页</a></div>';
        	}
        	$pagestr.='<div class="qieleft"><a href='.$url.'p='.($this->page-1).' title="上一页"> <<上一页</a></div>';
        	$pagestr.='<ul>';
        }
        ...
        for($i=$start;$i<=$end;$i++){
        	if($this->page == $i){
        		$pagestr.="<li><a href=".$url."p=".$i.' title='.$i.' style="background: #ff6600 none repeat scroll 0 0;color: #ffffff;"'.'>'.$i."</a></li>";
        	} else {
        ...
        if($this->page!= $this->pagenum){
        	$pagestr.='</ul>';
        	$pagestr.='<div class="qieleft"><a href='.$url.'p='.($this->page + 1).' title="下一页">下一页>> </a></div>';
        	if($this->page+5<$this->pagenum){
        		$pagestr.='<div class="qieleft"><a href='.$url."p=".$this->pagenum.'>尾页</a></div>';
        	}
        	$pagestr.='<div class="tiao">';
        	$pagestr.='<span>';
        	$pagestr.='共'."$this->pagenum".'页/'."$this->count".'条</span>';
        	$pagestr.='<input type="hidden" id="pagecount" value='."$this->pagenum".' />';
        	$pagestr.='<input type="text" id="go" name="p" value='."$this->page".' class="inps" />';
        	$pagestr.='<input type="buntton" value="GO" class="sub"/>';
        	$pagestr.='</div></div></div>';
        }
