...
class MenuController extends AdminBaseController {
	...
	public function admin_add(){
		...
		if(IS_POST){
			...
			$value['groupid'] = $this->I_chk('post.groupid','所在类别',0,0);
			$value['upid'] = $this->I_chk('post.upid','上级导航',0,0);
			$value['name'] = $this->I_chk('post.name','名称',0,0);
			$value['title'] = $this->I_chk('post.title','title标签',0,0);
			$value['alt'] = $this->I_chk('post.alt','alt标签',0,0);
			$value['link'] = $this->I_chk('post.link','链接地址',0,0);
			$value['about'] = $this->I_chk('post.about','链接说明',0,0);
