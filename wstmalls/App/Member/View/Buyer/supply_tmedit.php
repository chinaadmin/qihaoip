...
<div class="hytit">
您当前的位置：<a href="{:U('Buyer/index')}">买家中心</a> > 商标求购
</div>
<div class="hgrzy">
<div class="col-xs-12 sell_creat">
<ul>
<li><a href="javascript:;" class="selcreat">商标求购</a></li>
</ul>
</div>
<div class="col-xs-12 fbzlform creat_border" id="yqljs0">
<form class="form-horizontal" id="retrade">
 <div class="form-group">
 <input type="hidden" name="uid" value="{$data['tm']['uid']}"/>
 <input type="hidden" name="type" value="trade"/>
	<div class="col-xs-12 scrzgmc fbqgxx1">
	    <font>*</font>求购标题：&nbsp;
	    <input type="text" name="tmname" class="form-control fbqgxx" placeholder="求购标题" value="{$data['tm']['title']}"/>
	</div>
   <div class="col-xs-12 scrzgmc fbqgxx1">
    <font>*</font>求购分类：&nbsp;
		<select class="form-control fbqgxx" name="tmcate">
		<volist name="data['cate']['sb']" id="vo">
		  <option <if condition="$vo['id'] eq $data['tm']['groupid']">selected = "selected"</if> value="{$vo['id']}">{$vo['name']}</option>
		</volist>
		</select>
  </div>
    <div class="col-xs-12 scrzgmc fbqgxx1">
    	价格要求：&nbsp;
    <input type="text" name="tmprice" class="form-control fbqgxx" placeholder="价格要求" value="{$data['tm']['price']}"/>不填则为“面议”
  </div>
  <div class="col-xs-12">
    <div class="col-xs-1 scrzgmc fbqgxx1"><font>*</font>过期日期：&nbsp;   </div>
    	<input type="text" name="tmendtime" class="form-control fbqgxx" id="J-xl" placeholder="过期日期" value="{$data['tm']['endtime']}"/>
		<select class="form-control fbqgxx" name="tmselecttime">
  ...
  <div class="col-xs-12 scrzgmc fbqgxx1">
    ...
    <textarea class="form-control fbqgxx" name="tminstruct" rows="3" placeholder="为了更好的展示您的求购，建议填写详细说明">{$data['tm']['content']}</textarea>
  </div>
   <div class="col-xs-12">
...
$('.tradequeding').click(function(){
	if($("input[name='tmname']").val() == ""){
		$.MsgBoxgj.Alerta("温馨提示：",'求购标题不能为空！',function(){
			$("input[name='tmname']").focus();
		});
		return false;
	}
	if($("input[name='tmcate']").val() == ""){
		$.MsgBoxgj.Alerta("温馨提示：",'求购分类不能为空！',function(){
			$("input[name='tmcate']").focus();
		});
		return false;
	}
	if(isNaN($("input[name='tmprice']").val())){
		$.MsgBoxgj.Alerta("温馨提示：",'价格必须为数字！',function(){
			$("input[name='tmprice']").focus();
		});
		return false;
	}
	if($("input[name='tmendtime']").val() == ""){
		$.MsgBoxgj.Alerta("温馨提示：",'过期日期不能为空！',function(){
			$("input[name='tmduetime']").focus();
		});
		return false;
	}	
	var trade = $('#retrade').serialize();
	$.post("{:U('Member/Buyer/supply_edit')}",trade,function(data){
		if(data.state == 1){
