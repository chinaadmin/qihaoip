...
class PayController extends AdminBaseController {
	...
	public function admin_ShowList(){
		...
		if(isset($search['orders'])){$this->_where['orders'] = $search['orders'];}
		if(isset($search['money'])){$this->_where['money'] = $search['money'];}
	...
	public function admin_add(){
		...
		if(IS_POST){
			...
			$value['buyer'] = $this->I_chk('post.buyer','买家支付账号',0,0);
			$value['buyid'] = $this->I_chk('post.buyid','卖家支付id',0,0);
			$value['addtime'] = $this->I_chk('post.addtime','创建时间',0,0);
