...
class MsgController extends AdminBaseController {
	...
	public function admin_ShowList(){
		...
		if(isset($search['toid'])){$this->_where['toid'] = $search['toid'];}
		if(isset($search['usertype'])){$this->_where['usertype'] = $search['usertype'];}
	...
	public function admin_add(){
		...
		if(IS_POST){
			...
			$value['uid'] = $this->I_chk('post.uid','用户id',0,0);
			$value['userid'] = $this->I_chk('post.userid','发送者',0,0);
			$value['toid'] = $this->I_chk('post.toid','接收者',0,0);
			$value['usertype'] = $this->I_chk('post.usertype','用户类型',0,0);
			$value['sendtime'] = $this->I_chk('post.sendtime','发送时间',0,0);
			$value['totime'] = $this->I_chk('post.totime','接收时间',0,0);
			$value['data'] = $this->I_chk('post.data','数据详情',0,0);
