...
class JifenController extends AdminBaseController {
	private $_where = array();//查询条件
	private $_data = array();//数据数组
	private $_db = 'jifen';//数据库
	private $_name = '';
	/***
	...
	public function admin_add(){
		...
		if(IS_POST){
			...
			$value['value'] = $this->I_chk('post.value','积分值',0,0);
			$value['datetimes'] = $this->I_chk('post.datetimes','每日积分次数',0,0);
			$value['membertimes'] = $this->I_chk('post.membertimes','每账号积分次数',0,0);
			$value['name'] = $this->I_chk('post.name','积分名称',0,0);
			$value['about'] = $this->I_chk('post.about','积分描述',0,0);
