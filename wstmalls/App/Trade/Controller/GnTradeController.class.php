...
class GnTradeController extends TradeBaseController {
    
	public function index(){
		$data['groupid'] = M('Items')->where(array('parentid'=>'0','tmpa'=>'1','state'=>'1'))->select();
		$user = M('user_trade')->where(array('user_id'=>get_userid(),'type'=>'1'))->select();
    	...
    	$year = M('user_trade')->field('sq_date')->where(array('user_id'=>get_userid(),'type'=>'1'))->group('trade_user')->select();
		...
		//echo  M('user_trade')->_sql();
		//die;
		foreach ($data['trade'] as $key=>$value){
			$re = M('item')->where(array('tmno'=>$value['reg_id'],'userid'=>get_userid()))->find();
			if($re){
				$data['trade'][$key]['trade_sell'] = 1;
			}
		}
	...
	public function tmimg($id, $class) {
		...
		header('Location: http://www.quandashi.com/member/new-image?brand='.$id.'&type=original');exit;
	...
	public function daochu(){
		$data = M('user_trade')->where(array('user_id'=>get_userid(),'type'=>1))->select();
		foreach ($data as $key=>$value){
	...
	public function ajax_jy(){
		...
		$check = M('item')->where(array('tmno'=>$id,'userid'=>get_userid(),'state'=>array('neq',8)))->count();
		if($check){
	...
	public function all_jy(){
		...
		if(IS_POST){
			...
			$check = M('item')->where(array('tmno'=>array('in',$data),'userid'=>get_userid(),'state'=>array('neq',8)))->count();
			if(!$check){
				$re = M('user_trade')->where(array('uid'=>array('in',$ids)))->save(array('price'=>$post['price'],'trading_state'=>1));
			}else{
