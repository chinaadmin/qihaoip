...
class OrdersController extends AdminBaseController {
	...
	public function admin_add(){
		...
		if(IS_POST){
			...
			$value['num'] = $this->I_chk('post.num','订单商品格式',0,0);
			...
			$value['payid'] = $this->I_chk('post.payid','支付流水号',0,0);
			$value['paydate'] = $this->I_chk('post.paydate','支付日期',0,0);
			$value['paytime'] = $this->I_chk('post.paytime','支付时间',0,0);
