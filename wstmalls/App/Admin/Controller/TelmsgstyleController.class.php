...
class TelmsgstyleController extends AdminBaseController {
	private $_where = array();//查询条件
	private $_data = array();//数据数组
	private $_db = 'telmsgstyle';//数据库
	private $_name = '';
	/***
	...
	public function admin_add(){
		...
		if(IS_POST){
			...
			$value['method'] = $this->I_chk('post.method','模板方法',0,0);
			$value['name'] = $this->I_chk('post.name','名称',0,0);
			$value['configs'] = $this->I_chk('post.configs','配置组',0,0);
			$value['data'] = $this->I_chk('post.data','模板内容',0,0);
