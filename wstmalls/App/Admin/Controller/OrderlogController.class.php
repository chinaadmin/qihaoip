...
class OrderlogController extends AdminBaseController {
	private $_where = array();//查询条件
	private $_data = array();//数据数组
	private $_db = 'orderlog';//数据库
	private $_name = '';
	/***
	...
	public function admin_add(){
		...
		if(IS_POST){
			...
			$value['orderid'] = $this->I_chk('post.orderid','订单id',0,0);
			$value['userid'] = $this->I_chk('post.userid','排序',0,4);
			$value['adminid'] = $this->I_chk('post.adminid','操作人',0,0);
			$value['admintype'] = $this->I_chk('post.admintype','操作人类型',0,0);
			$value['datetime'] = $this->I_chk('post.datetime','处理时间',0,0);
			$value['type'] = $this->I_chk('post.type','事由id',0,0);
			$value['name'] = $this->I_chk('post.name','订单事由',0,0);
			$value['about'] = $this->I_chk('post.about','事由详情',0,0);
			$value['file'] = $this->I_chk('post.file','相关文件',0,0);
