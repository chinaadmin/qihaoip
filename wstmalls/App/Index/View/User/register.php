				...
				<form action="#" method="post" id="documentForm"
					...
					<div class="biaodan">
						<div class="biaodans">
							...
							<input type="password" name="password" id="mima" class="inputs"
					...
					<div class="biaodan">
						<div class="biaodans">
							<span class="glyphicon glyphicon-lock" aria-hidden="true"></span>
							<input type="password" name="password1" id="dbmima"
								class="inputs" placeholder="重复密码" required />
						</div>
						<span>两次密码不一致</span>
					</div>
					<div class="biaodan">
						<div class="biaodant">
							...
							<input type="text" name="pin" id="yzm" class="inputs"
								placeholder="验证码" required />
						</div>
						<a href="javascript:" id="kkks" onclick="send_pin()">发送验证码至手机/邮箱</a>
					</div>
					<div class="fwtk">
						<p>
							<input type="checkbox" checked="checked" class="agreed"
								name="toyi" value="agree" />
							同意
							<a href="__APP__/news/20150522/1202HTMLSUFFIX"
								target="_blank">7号网服务条款</a>
						</p>
						<span>
							<a href="<?php echo U('User/login');?>" target="_blank">已有帐号？马上登录</a>
						</span>
					</div>
					<div class="zhucet">
						<button type="submit" class="btns">注 册</button>
					</div>
				</form>
			</div>
		</div>
...
function chk_form(){
	...
	if($('.agreed').attr('checked')!='checked'){
		$.MsgBox.Alert("温馨提示：",'注册网站需要同意7号网服务条款！');
		return false;
	}
	return true;
}
