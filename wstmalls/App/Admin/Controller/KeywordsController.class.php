...
class KeywordsController extends AdminBaseController {
	...
	public function admin_ShowList(){
		...
		if(isset($search['keywords'])){$this->_where['keywords'] = $search['keywords'];}
	...
	public function admin_add(){
		...
		if(IS_POST){
			...
			$value['groupid'] = $this->I_chk('post.groupid','关键词组',0,0);
			$value['keywords'] = $this->I_chk('post.keywords','搜索关键词',0,0);
			$value['views'] = $this->I_chk('post.views','搜索次数',0,0);
