...
class MoneylogController extends AdminBaseController {
	...
	public function admin_add(){
		...
		if(IS_POST){
			...
			$value['userid'] = $this->I_chk('post.userid','用户账号',0,0);
			$value['orderid'] = $this->I_chk('post.orderid','订单号',0,0);
			$value['payid'] = $this->I_chk('post.payid','支付号',0,0);
			$value['name'] = $this->I_chk('post.name','异动名称',0,0);
			$value['type'] = $this->I_chk('post.type','异动类型',0,0);
			$value['date'] = $this->I_chk('post.date','发生日期',0,0);
			$value['datetime'] = $this->I_chk('post.datetime','发生时间',0,0);
			$value['money'] = $this->I_chk('post.money','异动金额',0,0);
			$value['beginmoney'] = $this->I_chk('post.beginmoney','开始余额',0,0);
			$value['endmoney'] = $this->I_chk('post.endmoney','结束金额',0,0);
			$value['adminid'] = $this->I_chk('post.adminid','操作员',0,0);
