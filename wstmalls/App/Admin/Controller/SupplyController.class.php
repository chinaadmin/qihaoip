...
class SupplyController extends AdminBaseController {
	...
	public function admin_ShowList(){
		...
		if(isset($search['supplytype'])){$this->_where['supplytype'] = $search['supplytype'];}
		...
		if(isset($search['pricemin'])){$this->_where['pricemin'] = $search['pricemin'];}
		if(isset($search['pricemax'])){$this->_where['pricemax'] = $search['pricemax'];}
		if(isset($search['edittime'])){$this->_where['edittime'] = $search['edittime'];}
		if(isset($search['endtime'])){$this->_where['endtime'] = $search['endtime'];}
	...
	public function admin_add(){
		...
		if(IS_POST){
			...
			$value['uid'] = $this->I_chk('post.uid','信息号',0,0);
			$value['supplytype'] = $this->I_chk('post.supplytype','供/求类型',0,0);
			$value['tmpa'] = $this->I_chk('post.tmpa','商标/专利',0,0);
			$value['groupid'] = $this->I_chk('post.groupid','产品类别',0,0);
			$value['userid'] = $this->I_chk('post.userid','用户id',0,0);
			$value['title'] = $this->I_chk('post.title','标题',0,0);
			$value['content'] = $this->I_chk('post.content','内容',0,0);
			$value['pricemin'] = $this->I_chk('post.pricemin','价格',0,0);
			$value['pricemax'] = $this->I_chk('post.pricemax','价格',0,0);
			$value['adddate'] = $this->I_chk('post.adddate','提交日期',0,0);
			$value['addtime'] = $this->I_chk('post.addtime','开始时间',0,0);
			$value['edittime'] = $this->I_chk('post.edittime','更新时间',0,0);
			$value['endtime'] = $this->I_chk('post.endtime','结束时间',0,0);
