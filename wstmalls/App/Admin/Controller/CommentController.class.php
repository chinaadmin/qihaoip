...
class CommentController extends AdminBaseController {
	...
	public function admin_ShowList(){
		...
		if(isset($search['artid'])){$this->_where['artid'] = $search['artid'];}
		if(isset($search['type'])){$this->_where['type'] = $search['type'];}
		if(isset($search['upid'])){$this->_where['upid'] = $search['upid'];}
		if(isset($search['amountid'])){$this->_where['amountid'] = $search['amountid'];}
		if(isset($search['amountip'])){$this->_where['amountip'] = $search['amountip'];}
	...
	public function admin_add(){
		...
		if(IS_POST){
			...
			$value['artid'] = $this->I_chk('post.artid','数据ID',0,0);
			$value['type'] = $this->I_chk('post.type','评论的类型',0,0);
			$value['upid'] = $this->I_chk('post.upid','上级评论',0,0);
			$value['amountid'] = $this->I_chk('post.amountid','评论者账号',0,0);
			$value['amountip'] = $this->I_chk('post.amountip','访客IP',0,0);
			$value['content'] = $this->I_chk('post.content','评论内容',0,0);
			$value['like'] = $this->I_chk('post.like','喜欢',0,0);
			$value['unlike'] = $this->I_chk('post.unlike','不喜欢',0,0);
