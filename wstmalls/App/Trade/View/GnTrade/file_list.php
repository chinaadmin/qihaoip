...
<div class="col-xs-12 ">
          ...
          <div class="zlyjjyst1">
            <form method="post" action="<?php echo U('Trade/GnTrade/file_list')?>">
				<div class="formdivs">
...
<form method="post" action="">
          <table cellpadding="0" cellspacing="0" class="tablesths">
			...
			<tr class="zlhover" id="<?php echo $value['uid']?>">
                ...
                <a href="<?php echo U('Trade/GnTrade/del_file',array('uid'=>$value['uid']))?>" class="tabaction tabactiont"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span></a></td>
              </tr>
			...
			<?php endforeach;?>
			  ...
			  <tr>
				...
				&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" class="btn btn-default adddel" url="<?php echo U('Trade/GnTrade/file_delete')?>">删除</a></div>
				</td>
             </tr>
            </tbody>
          </table>
...
<script type='text/javascript'>
 $('.show_img').live('click',function(){
	 ...
	 if($(this).parents('tr').next().find('.'+uid).length){
		...
		$.post("<?php echo U('Trade/GnTrade/ajax_get_file')?>",{'uid':uid},function(msg){
	...
	$.ajax({
		   ...
		   success: function(msg){
				...
				$.MsgBoxgj.Alerta('温馨提示',msg.msg,function(){
					if(msg.code==1){
						$.post("<?php echo U('Trade/GnTrade/ajax_get_file')?>", {'uid':msg.uid}, function(data){
...
$(".posts4").live('click',function(){
	var id = $(this).attr('id');
	var obj = $(this);
	var url = "<?php echo U('Trade/GnTrade/show_file')?>";
